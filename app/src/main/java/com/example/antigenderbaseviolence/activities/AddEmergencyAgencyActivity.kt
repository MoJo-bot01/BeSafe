...
import com.example.antigenderbaseviolence.databinding.ActivityAddEmergencyAgencyBinding
...
//view binding
private lateinit var binding: ActivityAddEmergencyAgencyBinding

class AddEmergencyAgencyActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityAddEmergencyAgencyBinding.inflate(layoutInflater)
    ...
    private fun validateData() {
        ...
        if (TextUtils.isEmpty(name)) {
            Toast.makeText(baseContext, "Enter The Emergency Agency Name...", Toast.LENGTH_SHORT).show()
        }else if (TextUtils.isEmpty(description)) {
            Toast.makeText(baseContext, "Enter The Emergency Agency Description...", Toast.LENGTH_SHORT).show()
        }else if (TextUtils.isEmpty(telephone)) {
            Toast.makeText(baseContext, "Enter The Emergency Agency Phone Number...", Toast.LENGTH_SHORT).show()
        } else if (!Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
            Toast.makeText(baseContext, "Invalid Email Address...", Toast.LENGTH_SHORT).show()
        }else{
            if (imageUri == null){
                Toast.makeText(baseContext, "Capture or Upload Emergency Agency Image...", Toast.LENGTH_SHORT).show()
            }
            else{
                uploadImage()
            }
    ...
    private fun uploadImage() {
        progressDialog.setMessage("Adding Emergency Agency...")
        progressDialog.show()

        val filePathAndName = "EmergencyAgencyImages/"+auth.uid
        ...
        ref.putFile(imageUri!!)
            .addOnSuccessListener { taskSnapshot->
                ...
                uploadEmergencyAgencyInfo(uploadedImageUrl)
            }
    }

    private fun uploadEmergencyAgencyInfo(uploadedImageUrl: String) {
        ...
        ref.child("$timestamp")
            ...
            .addOnSuccessListener {
                ...
                startActivity(Intent(this@AddEmergencyAgencyActivity, GetHelpAdminActivity::class.java))
                finish()
            }
