...
class AdapterForums : RecyclerView.Adapter<AdapterForums.HolderForums>{
    ...
    private fun moreOptionDialog(model: ModelForums, holder: HolderForums) {
        ...
        val options = arrayOf("Report Forum")

        // Create and show the AlertDialog
        val builder = AlertDialog.Builder(context)
        builder.setTitle("Choose Option")
            .setItems(options) { _, _ ->
                    showReportConfirmationDialog(id)
            }
            .show()
    }

    private fun showReportConfirmationDialog(id: String) {
        val builder = AlertDialog.Builder(context)
        builder.setTitle("Report Forum")
            .setMessage("Are you sure you want to report this forum?")
    ...
    private fun reportForum(id: String) {
        val ref = FirebaseDatabase.getInstance().getReference("Forums")
        val forumRef = ref.child(id)

        // Get the current report count
        forumRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                val currentReports = snapshot.child("reports").getValue(Int::class.java) ?: 0

                // Increment the report count by 1
                val updatedReports = currentReports + 1

                // Update the report count in the database
                forumRef.child("reports").setValue(updatedReports)
                    .addOnSuccessListener {
                        // Report count updated successfully
                        notifyDataSetChanged() // Refresh the RecyclerView
                        Toast.makeText(context, "Forum reported", Toast.LENGTH_SHORT).show()
                    }
                    .addOnFailureListener { e ->
                        // Handle failure to update report count
                        Toast.makeText(context, "Failed to report forum: ${e.message}", Toast.LENGTH_SHORT).show()
                    }
            }
